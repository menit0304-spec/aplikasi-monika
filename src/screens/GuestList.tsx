...
export default function GuestList({ 
  ...
  onPaymentClick?: (name: string) => void,
              ...
              {guestStatusTabs.map(tab => (
                <button 
                  ...
                  className={cn(
                    ...
                    currentTab === tab ? "bg-white text-primary shadow-sm" : "text-on-surface-variant hover:text-primary/60"
                  )}
                >
                  {tab}
                </button>
              ...
              >
                ...
                <div className="col-span-1 md:col-span-3 flex items-center gap-5">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[10px] font-bold text-outline/40 uppercase tracking-widest truncate">ID: {guest.id_number || guest.idNumber || "---"}</p>
                  </div>
                </div>
                ...
                <div className="col-span-1 md:col-span-2 flex flex-col items-center justify-center">
                  ...
                  <div className="flex items-center gap-2 group-hover:text-primary transition-colors">
                     ...
                     <span className="text-xs font-bold font-mono">{guest.phone_number || guest.phoneNumber || "---"}</span>
                  </div>
                  <p className="hidden md:block text-[9px] font-bold text-outline/40 mt-1 truncate max-w-full opacity-60">ID: {guest.id_number || guest.idNumber}</p>
                </div>
                ...
                <div className="col-span-1 md:col-span-2 flex flex-col items-center justify-center">
                  ...
                  <div className="flex items-center gap-2 bg-primary/5 px-4 py-1.5 rounded-full border border-primary/10 group-hover:bg-primary group-hover:text-white transition-all">
                    ...
                    <span className="text-sm font-black text-primary group-hover:text-white">{guest.roomNumber || guest.room_number || "--"}</span>
                  </div>
                  <p className="hidden md:block text-[9px] font-bold text-outline/60 mt-1 uppercase truncate max-w-full">{guest.roomType || guest.room_type}</p>
                </div>
                ...
                <div className="col-span-1 md:col-span-2 flex md:flex-col items-center justify-center gap-3 md:gap-0">
                   ...
                   <div className="flex flex-col items-center">
                      <span className="text-[10px] font-bold text-on-surface">{guest.checkIn || guest.check_in}</span>
                      <div className="h-2 w-px bg-outline-variant/30 hidden md:block my-0.5" />
                      <span className="text-[10px] font-bold text-on-surface">{guest.checkOut || guest.check_out}</span>
                   </div>
                </div>
                ...
                <div className="col-span-1 md:col-span-1 flex items-center justify-end gap-2 translate-y-2 md:translate-y-0 border-t md:border-0 pt-4 md:pt-0 border-outline-variant/10">
                  <button 
                    onClick={() => onPaymentClick?.(guest.name)}
                    disabled={isGuest}
                    className={cn(
                      "w-10 h-10 flex items-center justify-center text-emerald-600 bg-emerald-50 hover:bg-emerald-100 rounded-xl transition-all",
                      isGuest && "opacity-30 cursor-not-allowed"
                    )}
                    title={isGuest ? "Mode Demo tidak bisa melakukan pembayaran" : "Pembayaran / Pelunasan"}
                  >
                    <Banknote size={18} />
                  </button>
